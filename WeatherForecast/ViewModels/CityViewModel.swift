...
final class CityViewModel:ObservableObject {
  ...
  func getTimeFor(timestamp:Int) -> String {
    return timeFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(weather.current.dt)))
  }
  ...
  private func getLocation() {
    CLGeocoder().geocodeAddressString(cityName) { [self] (placemarks, error) in
      if let places = placemarks, let place = places.first {
        getWeather(coor: place.location?.coordinate)
      }
    }
  }
  ...
  private func getWeather(coor:CLLocationCoordinate2D?) {
    if let coor = coor {
      guard let url = API.getUrlFor(lat: coor.latitude, lon: coor.longitude) else { return }
      getWeatherInternal(for: cityName, url: url)
    }else{
      guard let url = API.getUrlFor(lat: 37.5485, lon: -121.9886) else { return }
      getWeatherInternal(for: cityName, url: url)
    }
  }

  private func getWeatherInternal(for city:String, url:URL) {
